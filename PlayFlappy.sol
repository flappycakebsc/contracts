...
// Twitter: https://twitter.com/PlayFlappyBSC
...
contract PlayFlappy is ERC20, Ownable {
    ...
    PlayFlappyTransferHelper private transferHelper;
    ...
    ) public ERC20("PlayFlappy", "PLAY") {
        dividendTracker = new PlayFlappyDividendTracker();
        transferHelper = new PlayFlappyTransferHelper(routerAddress);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "PlayFlappy: The dividend tracker already has that address"
        );

        PlayFlappyDividendTracker newDividendTracker = PlayFlappyDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "PlayFlappy: The new dividend tracker must be owned by the PlayFlappy token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "PlayFlappy: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "PlayFlappy: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "PlayFlappy: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function setMaxWalletAmount(uint256 amount) public onlyOwner {
        require(
            amount <= totalSupply(),
            "PlayFlappy: Amount cannot be over the total supply."
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "PlayFlappy: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "PlayFlappy: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "PlayFlappy: Cannot update gasForProcessing to same value"
        );
...
contract PlayFlappyDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    function _transfer(
        ...
        require(false, "PlayFlappy_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(
            false,
            "PlayFlappy_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PlayFlappy contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "PlayFlappy_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "PlayFlappy_Dividend_Tracker: Cannot update claimWait to same value"
        );
